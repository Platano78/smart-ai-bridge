...
async function testSpawnSubagent() {
  ...
  try {
    ...
    // Verify subagent executed successfully
    if (!parsed.success) {
      throw new Error(`Subagent failed: ${parsed.error || 'unknown'}`);
    }

    // Verify role and response
    if (parsed.role !== 'planner') {
      throw new Error(`Expected role=planner, got ${parsed.role}`);
    }

    if (!parsed.response || parsed.response.length === 0) {
      throw new Error('Subagent returned empty response');
    }

    console.log('  ✅ spawn_subagent tool works');
    console.log('  ✅ Planner role executed');
    console.log(`  ✅ Response generated (${parsed.response.length} chars)`);
    TESTS.passed++;
  } catch (error) {
    console.log(`  ❌ FAILED: ${error.message}`);
    TESTS.failed++;
