...
**Ready for unlimited token development with enhanced DeepSeek!**
      ...
      case 'query_deepseek_with_file': {
        ...
        if (!fileResult.success) {
          // If file is too large for normal reading, try intelligent analysis
          if (fileResult.error.includes('File too large')) {
            console.error('🔄 File too large for direct analysis, switching to intelligent chunking...');
            ...
            try {
              ...
              return {
                content: [{
                  type: 'text',
                  text: `**🧠 Intelligent Analysis Complete (Strategy: ${intelligentResult.strategy}):**\n\n${JSON.stringify(intelligentResult, null, 2)}\n\n*Large file processed successfully with ${intelligentResult.strategy} strategy*`
                }]
              };
            } catch (error) {
              return {
        ...
        try {
          ...
          switch (result.strategy) {
            case 'direct':
              ...
              formattedResult += `*File Size: ${result.metadata.fileSize} bytes | Token Estimate: ${result.metadata.tokenEstimate}*`;
              break;
            ...
            case 'chunked':
              ...
              result.chunks.forEach((chunk, i) => {
                formattedResult += `- Chunk ${chunk.chunkIndex} (${chunk.chunkType}): Analyzed\n`;
              });
              formattedResult += `\n*Total Chunks: ${result.metadata.totalChunks} | File Size: ${result.metadata.fileSize} bytes*`;
              break;
            ...
            case 'strategic':
              ...
              formattedResult += `- Lines: ${result.structuralOverview.linesOfCode}\n\n`;
