 ...
 *
 * Adapted from MKG V2 for Smart AI Bridge v1.3.0
 * @source mecha-king-ghidorah-v2/src/backends/backend-registry.js
 */

/**
 * Default backend configurations for Smart AI Bridge
 * Note: User-configurable - these are defaults, not hardcoded requirements
 */
const DEFAULT_BACKENDS = {
  ...
  gemini: {
    ...
    enabled: true,
    priority: 2,
    description: 'Google Gemini Enhanced (32K context)',
    config: {}
  },
  deepseek: {
    type: 'deepseek',
    enabled: true,
    priority: 3,
    description: 'NVIDIA DeepSeek V3.1 Terminus (reasoning, 8K tokens)',
    config: {}
  },
  qwen: {
    type: 'qwen',
...
class BackendRegistry {
  ...
  register(name, backendConfig) {
    ...
    console.error(`[BackendRegistry] Registered backend: ${name} (type: ${type}, priority: ${priority})`);
  ...
  /**
   * Set adapter for a backend
   * Note: Adapters are set externally after creation
   * @param {string} name - Backend name
   * @param {Object} adapter - Backend adapter instance
   */
  setAdapter(name, adapter) {
    this.adapters.set(name, adapter);
    console.error(`[BackendRegistry] Set adapter for ${name}`);
  }
