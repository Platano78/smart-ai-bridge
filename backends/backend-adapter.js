 ...
 *
 * Ported from MKG V2 for Smart AI Bridge v1.3.0
 * @source mecha-king-ghidorah-v2/src/backends/backend-adapter.js
 */
...
class BackendAdapter {
  ...
  constructor(config) {
    ...
    /** @type {number} Failures before opening circuit */
    this.circuitOpenThreshold = 5;

    /** @type {number} Time before attempting reset (ms) */
    this.circuitResetTimeout = 30000;

    /** @type {number|null} When circuit was opened */
  ...
  /**
   * Make a request to the backend (must be implemented by subclass)
   * @abstract
  ...
  /**
   * Check backend health (must be implemented by subclass)
   * @abstract
